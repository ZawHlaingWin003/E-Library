        ...
        <div class="form-group w-75 mx-auto my-5">
            ...
            <div class="card p-3">
                <form action="{{ route('admin.books.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="form-group mb-4">
                        <label for="slug" class="form-label">Book Slug (Please focus & wait to generate a unique
                            slug)</label>
                        <input type="text" name="slug" id="slug"
                            class="form-control @error('slug') is-invalid @enderror" value="{{ old('slug') }}" autofocus>
                        @error('slug')
                            <small><span class="text-danger">* {{ $message }}</span></small>
                        @enderror
                    </div>
